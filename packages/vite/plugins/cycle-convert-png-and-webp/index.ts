...
async function _recursiveFindPicDir (ext: SupportExt, rootDirPath: string, relativeFilepathExistsMap: Record<string, 1>, dirPath: string) {
  ...
  for (let i = 0; i < lstatList.length; i++) {
    ...
    const [, noExtFilename, fileExt] = (lstat.name.match(new RegExp(`(.+)\\.(.+)$`)) || []) as (string | undefined)[]
...
function _getPathInfo (ext: SupportExt, rootDirPath: string, dirPath: string, noExtFilename: string) {
  const anotherExt = ext === 'png' ? 'webp' : 'png'
  const anotherDirPath = `${rootDirPath}${SL}${anotherExt}${dirPath.substring(rootDirPath.length + ext.length + 1)}`
...
async function _checkCreateAnotherPic (ext: SupportExt, rootDirPath: string, dirPath: string, noExtFilename: string) {
  ...
  try {
    ...
    if (ext === 'png') {
      await sharp(self.filepath)
        .webp()
        .toFile(another.filepath)
    } else {
      await sharp(self.filepath)
        .toFormat('png')
        .toFile(another.filepath)
...
function _getPathInfoParameters (options: CycleConvertPngAndWebpOptions, filepath: string) {
  ...
  const [, noExtFilename, fileExt] = (filename.match(new RegExp(`(.+)\\.(.+)$`)) || []) as (string | undefined)[]
