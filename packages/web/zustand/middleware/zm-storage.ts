...
type SetStateAction<T> = T | Partial<T> | {
	_(state: T): T | Partial<T>;
}

const zmStorage = <T>(initializer: StateCreator<T>, storageKeyMap?: StorageKeys<T>) => <API extends StoreApi<any> = StoreApi<T>>(set: API['setState'], get: API['getState'], store: API) => {
	store.setState = (setStateAction: SetStateAction<T>, replace?: boolean | undefined) => {
		const newState: Partial<T> = typeof setStateAction === 'function'
			? setStateAction(get())
			: setStateAction
