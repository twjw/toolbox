...
type Storage2Define<Prefix extends string, State extends Storage2PropState> = {
	[K in keyof State]: Omit<State[K], 'driver'> & {
		driver: Storage
		storageKey: K extends string ? Storage2ConnectString<Prefix, K> : never
	}
}
...
type Listener<State extends Storage2PropState> = <
	K extends keyof State = keyof State,
	T = State[K]['defaultValue'],
>(
	key: K,
	value: T,
	prevValue: T,
...
class Storage2<Prefix extends string, PropState extends Storage2PropState> {
	...
	static string(
		defaultValue: string,
		driver?: Storage,
	): Storage2DefineResult<string, Storage2TypeEnum.string> {
		return {
			defaultValue,
	...
	static number(
		defaultValue: number,
		driver?: Storage,
	): Storage2DefineResult<number, Storage2TypeEnum.number> {
		return {
			defaultValue,
	...
	static boolean(
		...
		driver?: Storage,
	): Storage2DefineResult<boolean, Storage2TypeEnum.boolean> {
		return {
			defaultValue,
	...
	static json<T>(
		defaultValue: T,
		driver?: Storage,
	): Storage2DefineResult<T, Storage2TypeEnum.json> {
		return {
			defaultValue,
	...
	protected setJsonValue<K extends keyof PropState>(
		define: Storage2Define<Prefix, PropState>[K],
		key: K,
		value: any,
	) {
	...
	protected parseJsonValue<T>(value: string | null, defaultValue: T): T {
		if (value == null || Storage2.emptyValues.includes(value)) return defaultValue
		...
		switch (currentDefine.type) {
			...
			case Storage2TypeEnum.boolean:
				return storageValue == null
					? currentDefine.defaultValue
					: Storage2.trueValues.includes(storageValue)
		}

		return currentDefine.defaultValue
	}

	update<K extends keyof PropState>(
		key: K,
		value: Storage2State<PropState>[K],
		isPublish = true,
	): Storage2State<PropState>[K] {
		const currentDefine = this.define[key]
