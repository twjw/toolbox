...
type MergeModulesMap<M extends EagerModule | ImportModule, R = M> = (
	modulePath: string,
	module: M,
) => R
...
function _mergeModules<M extends EagerModule | ImportModule, R = M>(
	ignorePrefixReg: RegExp,
	modules: Record<string, M>[],
	map?: MergeModulesMap<M, R>,
) {
	...
	for (let i = 0; i < modules.length; i++) {
		...
		for (const modulePath in currentModuleMap) {
			...
			moduleMap[noPrefixPath] = map
				? map(noPrefixPath, currentModuleMap[modulePath])
				: currentModuleMap[modulePath]
		}
	}
...
function registerPageRoutes({
	...
	const isOutletAll = new RegExp(`^${ignorePrefixStr}${bracketOutlet}`).test(
		Object.keys(pageModules?.[0] || {})[0] || '',
	)
	...
	const pageModuleMap = _mergeModules<ImportModule, { meta: any; module: ImportModule }>(
		ignorePrefixReg,
		pageModules,
		(modulePath, module) => {
