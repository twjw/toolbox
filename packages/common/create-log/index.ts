...
function _commonLog(prefix: string, type: LogType): CommonLogReturnFunction {
	...
	if (type === 'info') {
		...
		banners.push('border-radius: 4px; padding: 0 4px; background-color: green; color: #fff;')
	} else if (type === 'warn') {
		banners.push('border-radius: 4px; padding: 0 4px; background-color: orange; color: #fff;')
	} else if (type === 'error') {
		banners.push('border-radius: 4px; padding: 0 4px; background-color: red; color: #fff;')
	}
